...
(define (get-max-index n min-max max power-3)
  (if (>= max n)
      min-max
      (begin
        (set! power-3 (* power-3 3))
        (set! max (+ max power-3))
        (get-max-index n (list (+ (- max power-3) 1) max)  max power-3)
      )
      )
  )

(define (generate-transformations n min max transformations)
  (if (>= min max)
      transformations
      (if (<= n (+ min (quotient (- max min) 3)))
          (generate-transformations n min (+ min (quotient (- max min) 3)) (append transformations '(1)))
          (if (<= n (- max (+ 1 (quotient (- max min) 3))))
              (generate-transformations n (+ 1 (+ min (quotient (- max min) 3))) (- max (+ 1 (quotient (- max min) 3))) (append transformations '(2)))
              (generate-transformations n (+ 1 (- max (+ 1 (quotient (- max min) 3)))) max (append transformations '(3)))
              )
          )
      )
  )
...
(define (get-transformations n)
  (define min-max (get-max-index n '() 1 1))
  (if (equal? '() min-max)
      min-max
      (generate-transformations n (car min-max) (cadr min-max) '())
      )
  )
...
(define (apply-functional-transformations Fs tuple)
  (foldl (lambda (function tuple) (function tuple)) tuple Fs)
  )
...
(define get-nth-tuple
  (lambda (n tuple transform1 transform2 transform3)
...
(define get-nth-ppt-from-matrix-transformations
  ...
  ; (get-nth-tuple n '(3 4 5) (lambda (tuple) (multiply T1 tuple)) (lambda (tuple) (multiply T2 tuple)) (lambda (tuple) (multiply T3 tuple)))
  (lambda (n) (get-nth-tuple n '(3 4 5) (lambda (tuple) (multiply T1 tuple)) (lambda (tuple) (multiply T2 tuple)) (lambda (tuple) (multiply T3 tuple))))
  )
...
(define get-nth-quadruple
  ...
  ; (get-nth-tuple n '(1 1 2 3) (lambda (tuple) (apply Q1 tuple)) (lambda (tuple) (apply Q2 tuple)) (lambda (tuple) (apply Q3 tuple)))
  (lambda (n) (get-nth-tuple n '(1 1 2 3) (lambda (tuple) (apply Q1 tuple)) (lambda (tuple) (apply Q2 tuple)) (lambda (tuple) (apply Q3 tuple))))
  )
