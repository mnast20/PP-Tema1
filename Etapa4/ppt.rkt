...
(define (BFS-traversal queue triplet)
  (let ((triplet1 (multiply T1 triplet)) (triplet2 (multiply T2 triplet)) (triplet3 (multiply T3 triplet)))
    (stream-append
     (stream (car queue))
     (stream-lazy
      (let ((queue (append queue (list triplet1 triplet2 triplet3))))
        (BFS-traversal (cdr queue) (cadr queue)))))))
...
(define (aux-pairs G H n)
  (stream-append
   (let iterate-through-G ((G G) (H H) (n n))
     (if (= n 0)
         empty-stream
         (stream-cons (cons (stream-first G) (stream-first H)) (iterate-through-G (stream-rest G) H (- n 1))
                      )))
   (stream-lazy (aux-pairs G (stream-rest H) (+ 1 n))))
  )
...
(define (odd-naturals n)
  (stream-cons n (odd-naturals (+ n 2)))
  )

(define gh-pairs-stream
  (stream-filter (lambda (pair) (= 1 (gcd (car pair) (cdr pair)))) (pairs (odd-naturals 1) (odd-naturals 3)))
  )
