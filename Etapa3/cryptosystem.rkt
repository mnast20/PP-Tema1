  ...
  (let ((quadruple (get-nth-quadruple n)))
    (map (lambda (x) (modulo x 27))
         (let ((e (cadr quadruple)) (f (caddr quadruple)))
           (list (a1 e f) (b1 e f) (c1 e f) (a2 e f) (b2 e f) (c2 e f) (a3 e f) (b3 e f) (c3 e f)))))
  )
...
(define (extend-key-to-size key size)
  (let extend-to-size ((key key) (size size) (result '()))
    (if (equal? size 0)
        result
        (if (>= size (length key))
            (extend-to-size key (- size (length key)) (append result key))
            (extend-to-size key 0 (append result (take key size)))
            )))
  )
...
(define (extend-key key size)
  (letrec
      ((extend-the-key
        (lambda (key size)
          (if (> size (length key))
              (extend-key-to-size key size)
              (truncate-the-key key size)
              )))
       (truncate-the-key
        (lambda (key size)
          (if (equal? size (length key))
              key
              (take key size)
              ))))
    (extend-the-key key size))
  )
...
(define (apply-function-codes message key function)
  (map (lambda (x) (modulo x 27)) (map function message (extend-key key (length message))))
  )

(define encrypt-codes
  (lambda (message key)
    (apply-function-codes message key +))
  )
(define decrypt-codes
  (lambda (crypted key)
    (apply-function-codes crypted key -))
  )
...
(define (apply-function-message message key function)
  (codes->message (let ((codes (message->codes message))) (function codes key)))
  )

(define encrypt-message
  (lambda (message key)
    (apply-function-message message key encrypt-codes))
  )
(define decrypt-message
  (lambda (message key)
    (apply-function-message message key decrypt-codes))
  )